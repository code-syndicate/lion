...
class UserBankAccountAdmin(admin.ModelAdmin):
    list_display = ("user", "account_number", "account_type",
                    "balance", "last_modified", "date_created")
    search_fields = ["balance", "account_number", "id"]
    list_filter = ["account_type", "last_modified",
                   "date_created", "last_modified", "balance"]
...
class IntlTransferRequestAdmin(admin.ModelAdmin):
    ...
    list_filter = ["user", "status", "swift_code",
                   "iban_code", "status", "account_number"]
